...
public:
	...
	Slot* create_keyword_message(Slot* receiver, std::string lower_or_cap, Slot* expCP, bool addSlots);
	...
	Slot* create_parent_slot(Slot* object, std::string parent, std::string son);
	Slot* create_nil();
	Slot* create_object();
	Slot* collect();
	...
	Slot* set_object_script(Slot* slot, std::string script, std::string msg_slot_name_extended, std::vector<std::string> args);
	Slot* create_user_method(Slot *method, Slot* expCP);
